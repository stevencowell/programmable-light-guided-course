...
- `teacher-resources.html` — a source-mapped, teacher-developed programme for local review with 23 visible `Teacher to confirm` decisions and Print / Save PDF

## Local build and checks

```powershell
node scripts/build-data.mjs
node scripts/build-teacher-resources.mjs
node scripts/validate-course.mjs
node scripts/serve.mjs
```

The content build reads the bounded Module 1-5 handoffs authored in signed-in ChatGPT one named theory section at a time. The teacher-resource build reads its checked handoff record. The release validator checks counts, student-facing question quality, source coverage, stale scaffold content, outcome mapping, visual records, plan integration, teacher-programme boundaries and local links.
